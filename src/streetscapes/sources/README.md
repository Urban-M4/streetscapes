...
This module of the `streetscapes` CLI handles **fetching metadata from image sources** (Mapillary, KartaView and Panoramax) and **downloading images** based on manifests. Metadata is stored in GeoParquet (or Parquet) format, enabling easy downstream spatial processing with tools like DuckDB or GeoPandas.
...
* **Separate Commands per Source**: Each source has its own CLI subcommand (`mapillary`, `kartaview`, `panoramax`) due to differences in API semantics (tiling vs. paging, authentication requirements, etc.).
...
* **Sources**: Each source (Mapillary, KartaView, Panoramax) implements `fetch_metadata` and provides a standardized manifest writer.
